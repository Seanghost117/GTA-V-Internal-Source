...
void Invisibility(Player selected, bool toggle)
{
	if (toggle == true)
	{
		Player selectedPlayer = PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(selected);
		ENTITY::SET_ENTITY_VISIBLE(selectedPlayer, false, 0);
	}
	else
	{
		Player selectedPlayer = PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(selected);
		ENTITY::SET_ENTITY_VISIBLE(selectedPlayer, true, 0);
	}
}
void tp_peds_to_waypoint()
{
	...
	for (int i = 0; i < PedFound; i++)
	{
		...
		if (ENTITY::DOES_ENTITY_EXIST(peds[OffsetID]) && PLAYER::PLAYER_PED_ID() != peds[OffsetID])
		{
			...
			{
				coords.z = 1000.0;
				WEAPON::GIVE_DELAYED_WEAPON_TO_PED(peds[OffsetID], 0xFBAB5776, 1, 0);
			}
			//do it
			teleport_to_coords(e, coords);
		}
...
void FireLoop(Player selected)
{
	Player selectedPlayer = PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(selected);
	Vector3 Pos = ENTITY::GET_ENTITY_COORDS(selectedPlayer, true);
	FIRE::ADD_EXPLOSION(Pos.x, Pos.y, Pos.z - 2, 12, 5, true, 0, 0);
}
